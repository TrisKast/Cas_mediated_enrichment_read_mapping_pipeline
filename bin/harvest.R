...
gstride <- as.integer(args[4])

target_proximity <- as.integer(args[5])

offtarget_level <- as.integer(args[6])

max_threads <- as.integer(args[7])

working_dir <- args[8]

r_results <- file.path(working_dir,"Analysis","R")
on_target <- file.path(working_dir, "Analysis","OnTarget")
off_target <- file.path(working_dir, "Analysis","OffTarget")
dir.create(r_results, showWarnings = FALSE, recursive=TRUE)
dir.create(on_target, showWarnings = FALSE, recursive=TRUE)
dir.create(off_target, showWarnings = FALSE, recursive=TRUE)

##### no more config beyond this point - please #####

referenceFile <- file.path(working_dir,"ReferenceData", reference)
...
mappedBam <- file.path(working_dir, "Analysis","Minimap2", paste0(study, ".bam"))
unmappedBam = file.path(working_dir, "Analysis","Minimap2", paste0(study, ".unmapped.quals"))
